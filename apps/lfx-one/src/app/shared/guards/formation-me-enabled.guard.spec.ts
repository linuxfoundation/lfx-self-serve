...
import { formationMeEnabledGuard } from './formation-me-enabled.guard';

describe('formationMeEnabledGuard', () => {
  ...
  const route: Route = { path: 'formations', data: { lens: 'me' } };
  const segments: UrlSegment[] = [];

  const runGuard = (): ReturnType<typeof formationMeEnabledGuard> => TestBed.runInInjectionContext(() => formationMeEnabledGuard(route, segments));
  ...
  it('fails closed to / when the provider never becomes ready', async () => {
    ...
    expect(waitForReady).toHaveBeenCalledWith({ guard: 'formationMeEnabledGuard', flag: FORMATION_ENABLED_FLAG });
    expect(router.parseUrl).toHaveBeenCalledWith('/');
    expect(result).toEqual({ redirected: '/' });
    expect(getBooleanFlag).not.toHaveBeenCalled();
  });
