...
export class NewsletterRendererService {
  ...
  /**
   * Render a CONTAINER block's chrome split around its `<slot name="children" />`
   * so the composer can mount a LIVE CDK drop list inside the container's own
   * card (a static `innerHTML` string auto-closes its open tags, which would push
   * the nested blocks OUTSIDE the card border — the exact divergence from the
   * sent email this avoids).
   *
   * Returns the outermost element's emitted `shellClass` / `shellStyle` (bind
   * them onto a REAL host element so the card box is a continuous DOM node), plus
   * the inner `before` / `after` HTML on either side of the slot. The composer
   * renders `before`, then the live drop list, then `after`, all inside the shell
   * host. All '' on the server / without a template / when the root isn't a
   * single element.
   */
  public renderContainerChrome(
    template: string | undefined,
    content: Record<string, unknown>,
    templateOf?: (blockType: string) => string | undefined,
    editMode = false
  ): { shellClass: string; shellStyle: string; before: string; after: string } {
    const empty = { shellClass: '', shellStyle: '', before: '', after: '' };
    if (!isPlatformBrowser(this.platformId) || !template) {
      return empty;
    }
    const nodes = parseTemplate(template);
    ...
    const mapping = TAG_MAP[root.tag];
    const shellClass = classList(root.attrs, mapping?.className);
    const shellStyle = mergedStyle(root.attrs, mapping?.style);
    // Render the shell's children with the children-slot collapsed to a unique
    // sentinel, then split the string there. The sentinel is a control char no
    // template or bound value can contain, so the split point is unambiguous.
    const sentinel = '\u0000nl-container-slot\u0000';
    const ctx: RenderCtx = { content, children: [], templateOf, editMode, slotSentinel: sentinel };
    const inner = root.children.map((n) => this.renderNode(n, ctx)).join('');
    const at = inner.indexOf(sentinel);
    if (at === -1) {
      return { shellClass, shellStyle, before: inner, after: '' };
    }
    return { shellClass, shellStyle, before: inner.slice(0, at), after: inner.slice(at + sentinel.length) };
  }
  ...
  private renderNode(node: TemplateNode, ctx: RenderCtx): string {
    ...
    if (tag === 'slot') {
      ...
      // Container-chrome split: emit the sentinel so `renderContainerChrome` can
      // cut the string here and mount a live drop list in the slot's place.
      if (name === 'children' && ctx.slotSentinel !== undefined) {
        return ctx.slotSentinel;
      }
...
interface RenderCtx {
  ...
  rawText?: boolean;
  /**
   * When set, a `<slot name="children" />` renders as this sentinel string
   * instead of its child blocks, so `renderContainerChrome` can split the
   * container's chrome at the slot and host a live drop list there.
   */
  slotSentinel?: string;
}
