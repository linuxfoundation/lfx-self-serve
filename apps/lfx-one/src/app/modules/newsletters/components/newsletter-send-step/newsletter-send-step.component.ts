...
// review component (newsletter-review.component.ts) render the same "when should this go out?" picker as two radio cards
...
// component and the review component show the same schedule panel in two different steps.
...
export class NewsletterSendStepComponent {
  ...
  // The radio cards read their selected state from the sendMode @Input and push clicks
  // into the form via selectSendMode() — an @Input mirror the parent keeps in sync with
  // the form, rather than a direct [formControl] binding like the date/time pickers
  // below. sendMode is a plain @Input mirror rather than an output, the same pattern
  // the select-button toggle used.
