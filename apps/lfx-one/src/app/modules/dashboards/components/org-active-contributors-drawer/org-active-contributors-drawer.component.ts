...
const DEFAULT_MONTHLY: OrgContributorsMonthlyResponse = { monthlyData: [], monthlyLabels: [], totalContributors: 0 };
const DEFAULT_DISTRIBUTION: OrgContributorsProjectDistributionResponse = { projects: [] };
...
export class OrgActiveContributorsDrawerComponent {
  ...
  protected readonly monthlyData: Signal<OrgContributorsMonthlyResponse> = computed(() => this.drawerData().monthly);
  protected readonly distributionData: Signal<OrgContributorsProjectDistributionResponse> = computed(() => this.drawerData().distribution);
  ...
  private initDrawerData(): Signal<{ monthly: OrgContributorsMonthlyResponse; distribution: OrgContributorsProjectDistributionResponse }> {
    ...
    return toSignal(
      toObservable(this.visible).pipe(
        ...
        switchMap((isVisible) => {
          ...
          return forkJoin({
            monthly: this.analyticsService.getOrgContributorsMonthly(accountId, foundationSlug),
            distribution: this.analyticsService.getOrgContributorsProjectDistribution(accountId, foundationSlug),
          }).pipe(
  ...
  private initDistributionChartData(): Signal<ChartData<'bar'>> {
    return computed(() => {
      ...
      return {
        ...
        datasets: [
          {
            data: projects.map((p) => p.contributorCount),
