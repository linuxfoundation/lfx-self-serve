 ...
 *
 * IDEMPOTENT, because the label this appends and the one the server folds inline are the same
 * bytes. `emailCtaUnlinkedLabel` gates on variant A's `copy.ctaUrl`, but both bodies are folded
 * through here -- and variant B is generated by its OWN `generateEmailCopy` call, so B's button
 * can be url-LESS (server folds the label into `body`) while A's was supplied-then-refused
 * (label non-empty). Appending unconditionally then rendered the call to action TWICE in B.
 *
 * The comparison is exact rather than heuristic: the server writes
 * `<div><strong>${escapeHtml(sanitizeDisplayText(text))}</strong></div>` and
 * `emailCtaUnlinkedLabel` already applies `sanitizeDisplayText`, so an already-folded label is
 * byte-identical to the fragment built here.
 */
function withUnlinkedCta(body: string, unlinkedLabel: string): string {
  ...
  return body.includes(fragment) ? body : `${body}${fragment}`;
}
