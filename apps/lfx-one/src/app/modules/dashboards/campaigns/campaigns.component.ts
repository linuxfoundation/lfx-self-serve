 ...
 *
 * IDEMPOTENT for an exact duplicate. `emailCtaUnlinkedLabel` gates on variant A's `copy.ctaUrl`,
 * but both bodies are folded through here -- and variant B is generated by its OWN
 * `generateEmailCopy` call, so B's button can be url-LESS (campaign-service folds the label into
 * `body`, filter `!section.url`) while A's was supplied-then-refused (label non-empty).
 * Appending unconditionally then rendered the call to action TWICE in B.
 *
 * The two operands must be compared in the SAME serialized form, never assumed byte-identical.
 * `body` has already passed through `stripResourceLoadingHtml` -> sanitize-html, which DECODES
 * `&#39;` and `&quot;` back to bare `'` and `"` on output, while `escapeHtml` here encodes them.
 * So a label like "Don't Miss Out" left `<strong>Don't...` in the body and `<strong>Don&#39;t...`
 * in the fragment, `includes` missed, and the call to action still doubled. Normalizing the
 * fragment through the SAME sanitizer makes the comparison hold for every label instead of only
 * for ones that happen to round-trip unchanged.
 *
 * SCOPE, deliberately: this suppresses an EXACT duplicate. When B's own folded label and A's
 * refused label differ in WORDING -- reachable, since A is generated with an `urgency-fomo` tone
 * and B with the plain stage prompt -- B still ships both. Closing that needs B's generation to
 * report whether its own button was folded inline, which is a larger change than this helper.
 */
...
export class CampaignsComponent {
   ...
   *
   * `abTestIsStageable` does read it directly, and deliberately -- see that signal for why the
   * gate judges the PRE-fold body.
   */
  ...
  protected readonly abTestIsStageable = computed<boolean>(
    ...
    // The PRE-fold body, deliberately. `abTestBodyHtmlBForSend` can carry a refused CTA label
    // that the operator never wrote, and `hasVisibleHtmlText` counts it as visible -- so gating
    // on the folded value would stage a variant B whose entire body IS that label.
   ...
   *
   * B has no NATIVE button of its own, but its `body` CAN already carry a folded CTA label:
   * when B's own generation returns a url-LESS button, campaign-service folds the label inline
   * (`!section.url`). That is why the fold-back has to be idempotent -- see `withUnlinkedCta`
   * for how the two operands are normalized before comparison, and for what that guard does
   * NOT cover (a duplicate whose wording differs from A's).
   */
