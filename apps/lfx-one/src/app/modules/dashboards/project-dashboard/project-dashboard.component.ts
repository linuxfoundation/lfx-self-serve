...
import { formatAnnouncementDateLabel, isFormationStageGate } from '@lfx-one/shared/utils';
...
export class ProjectDashboardComponent {
  ...
  /**
   * GH-1958: the entry card only renders for a project in a "Formation - *" stage, dark-launched
   * behind `formation-enabled`. Uses the strict `isFormationStageGate` (excludes `Draft`), not the
   * loose `isFormationStage` (GH-1955 badge/card check) — the card links directly into the
   * `/foundation/formations` route, which is gated by the strict check via `formationsQueueAuditorGuard`,
   * so a `Draft`-stage project would otherwise render a card that dead-ends at the route guard.
   */
  protected readonly showFormationEntryCard = computed(() => this.formationEnabled() && isFormationStageGate(this.projectContextService.activeProjectStage()));
