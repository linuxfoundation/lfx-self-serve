...
function buildOrgsRouter(): Router {
  ...
  // GH-1655 — company-emails lookup for tabs (Board/Committee) whose rows have no personKey. Same guard rationale as above ('company-emails' must not be consumed as a personKey).
  router.get('/:orgUid/lens/people/company-emails', (req, res, next) => orgLensPeopleController.getCompanyEmails(req, res, next));
