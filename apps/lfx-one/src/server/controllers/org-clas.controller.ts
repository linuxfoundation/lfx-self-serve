...
export class OrgClasController {
  ...
  //
  // Invalidates one specific acknowledgment. Blocked during impersonation at the route, so there
  // is no impersonation branch here. Optional `reason` and `note` in the body — empty strings are
  // elided by the service before the upstream call.
  public async invalidateAcknowledgment(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    try {
      ...
      const input: OrgClaInvalidateAcknowledgmentInput = {
        // Trimmed inputs are elided by the service; a non-string reason/note is dropped here
        // (the service is defensive too, but this shape-check keeps the intent obvious).
