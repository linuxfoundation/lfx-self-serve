...
describe('ProfileAuthService.getAuthorizationUrl', () => {
  ...
  it('propagates a rejected Valkey write instead of minting a URL with an unissued nonce', async () => {
    const error = new Error('valkey unavailable');
    authStateSvc.issue.mockRejectedValue(error);
    const req = buildReq({ oidc: { user: { sub: 'user-1' } } });

    await expect(service.getAuthorizationUrl(req, '/profile')).rejects.toThrow(error);
  });
});
