...
import { ORG_EASYCLA_RETURN_SIGNED_PARAM, ORG_EASYCLA_RETURN_SIGNED_VALUE } from '@lfx-one/shared/constants';
...
export class OrgClaService {
  ...
  public async requestCorporateSignature(req: Request, orgUid: string, request: OrgClaSignRequest): Promise<OrgClaSignResponse> {
    ...
    if (request.sendAsEmail) {
      ...
      //
      // The organization is in the path (#2743): `/org/{sfid}/easycla/{group}`. `orgUid` is the
      // value the grant check already cleared and the same one sent as `company_sfid`, so the
      // address describes the session that was actually opened. SFID rather than slug so the mint
      // does not need a lookup; the path-param guard canonicalizes to the slug on return. The
      // leftover `?org=` reader stays for in-flight envelopes minted before this release.
      // The signed flag, because the row will not be on the list the instant they arrive —
      // without it the page would read a group with no signed agreement and settle straight onto
      // the cannot-preview state.
      body = {
        project_sfid: request.projectSfid,
        company_sfid: orgUid,
        return_url: claReturnUrl(req, `/org/${encodeURIComponent(orgUid)}/easycla/${encodeURIComponent(request.claGroupId)}`, {
