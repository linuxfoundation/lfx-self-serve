...
export class AkritesServerService {
  ...
  public mapListItem(item: CdpStewardshipSummary): AkritesPackage {
    ...
    return {
      ...
      stewards: (item.stewards ?? []).map((s) => ({ userId: s.userId, role: s.role, assignedAt: s.assignedAt, name: null, avatarUrl: null })),
      ...
      dependentRepos: null,
      scoreCardScore: item.scorecardScore != null ? `${item.scorecardScore.toFixed(1)} / 10` : null,
