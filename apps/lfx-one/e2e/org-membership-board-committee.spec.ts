 ...
 * so the AGL fixture data is what every test below asserts against — Alex Rivera,
 * Jordan Kim × 3, Taylor Morgan, Masaki Isetani × 2, Mitsuo Date.
 */
...
test.describe('US1 — Board & Committee tab view (FR-002 through FR-007)', () => {
  ...
  test('Committee Seats — Role-before-Committee column order + 2 foundation-controlled rows (FR-005, FR-004b)', async ({ page }) => {
    ...
    await expect(page.getByTestId('board-committee-committee-why-agl-com-1')).toBeVisible(); // Alex / TSC Chair
    await expect(page.getByTestId('board-committee-committee-edit-agl-com-1')).toHaveCount(0);
    await expect(page.getByTestId('board-committee-committee-why-agl-com-3')).toBeVisible(); // Jordan / Budget & Finance Observer
    ...
    await expect(page.getByTestId('board-committee-committee-edit-agl-com-2')).toBeVisible(); // Jordan / Marketing Member
...
test.describe('US2 — Reassign Board Roles modal (FR-008)', () => {
  ...
  test('Cancel button closes modal with no toast or table mutation (FR-008i, SC-005)', async ({ page }) => {
    ...
    await expect(page.getByTestId('board-committee-board-row-agl-board-1')).toContainText('Alex Rivera');
  });
