...
- [x] T004 [P] Add response-only `marketingAuditor?: boolean` and `campaignManager?: boolean` to `Project` in `packages/shared/src/interfaces/project.interface.ts`, documenting the fail-closed semantics (`false` = no grant or transient probe failure; `undefined` = probe not requested) per `data-model.md`.
- [x] T005 Add probe-gated marketing enrichment to `getProjectById` in `apps/lfx-one/src/server/services/project.service.ts`: when requested, check `marketing_auditor` and `campaign_manager` for the project UID in one batched round-trip via `AccessCheckService.checkAccessOrdered` (positional results — the id-keyed `checkAccess` map would collapse two relations on one uid) and map results to the camelCase `marketingAuditor` / `campaignManager` fields (mirror the `meetingCoordinator` block; leave `undefined` when not requested). Depends on T003, T004.
