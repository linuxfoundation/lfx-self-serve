...
/**
 * One block of a generated email body.
 *
 * Kept as a discriminated-ish record rather than a union of concrete section types because the
 * generator emits a small, open set (rich_text, button, image, spacer) and an unknown `type` must
 * survive a round-trip rather than be dropped — a section this app cannot render is still one the
 ...
 * generation; on a refine it is the ONLY thing that differs, so the same endpoint serves both and
 * there is no second "refine" route to keep in step with this one.
 */
export interface GenerateEmailCopyRequest {
  eventUrl: string;
  eventName: string;
  campaignGoal?: string;
  targetAudience?: string;
...
/**
 * The AI-generated email copy for a brief — the `email-copy` half of an email brief.
 *
 * SEPARATE from the brief itself because the two are generated by different calls and at
 * different times: the brief comes from the scrape on the Plan tab, the copy is generated on
 * demand in Implement and can be regenerated without re-scraping. Folding them together would
 * make a refine re-run the scrape.
 *
 * `bodyHtml` is the email body alone; `previewHtml` wraps it in the surrounding chrome for
 * display. Both are kept because staging sends the body while the UI shows the preview — deriving
 * one from the other at render time would put layout logic in two places.
 */
