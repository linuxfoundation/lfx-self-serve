...
/** Rotate right, on the unsigned 32-bit value. */
 ...
 *
 * `TextEncoder` is present in every runtime this package targets (browsers, Node 11+), and using it
 * rather than a hand-rolled encoder matters for correctness: a naive `charCodeAt` loop mis-encodes
 * anything outside the BMP, which would make the digest disagree with every other SHA-256.
 */
...
export function sha256Hex(input: string): string {
  ...
  // Padded length: the message, the mandatory 0x80 byte, zero padding, and an 8-byte length, rounded
  ...
  // 64-bit big-endian bit count in the final eight bytes. Written as a high/low pair because a
  // single `setUint32` cannot express lengths past 2^32 bits, and `setBigUint64` would drag BigInt
  // into a hot path for no benefit at these sizes.
  const view = new DataView(padded.buffer);
  view.setUint32(paddedLength - 8, Math.floor(bitLength / 0x100000000), false);
  view.setUint32(paddedLength - 4, bitLength >>> 0, false);

  // Initial hash values: the first 32 bits of the fractional parts of the square roots of the first
  // eight primes.
  let h0 = 0x6a09e667;
  let h1 = 0xbb67ae85;
  let h2 = 0x3c6ef372;
