...
describe('classifyCommitteeEngagement — decision table', () => {
  ...
  it('classifies Emeritus regardless of a low real attendance rate', () => {
    expect(classifyCommitteeEngagement({ attended: 1, invited: 20, votingStatus: EMERITUS, role: undefined, joinedWithinWindow: false })).toBe('Emeritus');
  });

  it('classifies Emeritus even for perfect attendance', () => {
    expect(classifyCommitteeEngagement({ attended: 10, invited: 10, votingStatus: EMERITUS, role: undefined, joinedWithinWindow: false })).toBe('Emeritus');
  });
  ...
  it('does not broaden the LF Staff carve-out to non-staff Observers', () => {
    expect(classifyCommitteeEngagement({ attended: 2, invited: 10, votingStatus: OBSERVER, role: undefined, joinedWithinWindow: false })).toBe('Low');
  });
  ...
  it('does not broaden the None carve-out beyond staff — a non-staff member with no voting status still classifies on attendance/tenure normally', () => {
    expect(classifyCommitteeEngagement({ attended: 0, invited: 0, votingStatus: NONE, role: undefined, joinedWithinWindow: true })).toBe('High');
    expect(classifyCommitteeEngagement({ attended: 0, invited: 3, votingStatus: NONE, role: undefined, joinedWithinWindow: false })).toBe('Inactive');
  });
  ...
  it('classifies High (not Inactive) for a member with zero invites who joined within the window', () => {
    expect(classifyCommitteeEngagement({ attended: 0, invited: 0, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: true })).toBe('High');
  });
  ...
  it('classifies Inactive for a never-invited member who has been on the roster the whole window', () => {
    expect(classifyCommitteeEngagement({ attended: 0, invited: 0, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: false })).toBe('Inactive');
  });

  // Rows 5-7: threshold on the real rate, unchanged boundaries.
  it('classifies a rate just below the medium threshold as Low', () => {
    expect(classifyCommitteeEngagement({ attended: 39, invited: 100, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: false })).toBe('Low');
  });

  it('classifies a rate exactly at the medium threshold as Medium', () => {
    expect(classifyCommitteeEngagement({ attended: 40, invited: 100, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: false })).toBe('Medium');
  });

  it('classifies a rate just below the high threshold as Medium', () => {
    expect(classifyCommitteeEngagement({ attended: 74, invited: 100, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: false })).toBe('Medium');
  });

  it('classifies a rate exactly at the high threshold as High', () => {
    expect(classifyCommitteeEngagement({ attended: 75, invited: 100, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: false })).toBe('High');
  });

  it('classifies perfect attendance as High', () => {
    expect(classifyCommitteeEngagement({ attended: 10, invited: 10, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: false })).toBe('High');
  });
  ...
  it('classifies a rate that only reaches the medium threshold after rounding as Low', () => {
    ...
    expect(classifyCommitteeEngagement({ attended: 395, invited: 1000, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: false })).toBe('Low');
  });
  ...
  it('classifies Inactive for a member invited at least once who attended nothing, even if joined within the window', () => {
    expect(classifyCommitteeEngagement({ attended: 0, invited: 3, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: true })).toBe('Inactive');
    expect(classifyCommitteeEngagement({ attended: 0, invited: 3, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: false })).toBe('Inactive');
  });
  ...
  it('the Orlin case: joined mid-window, invited 5, attended 5, classifies High at 100% and is never at-risk', () => {
    const input = { attended: 5, invited: 5, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: true };
...
describe('isCommitteeMemberAtRisk', () => {
  it('is false for a never-invited member who joined within the window (Inactive is unreachable there)', () => {
    expect(isCommitteeMemberAtRisk({ attended: 0, invited: 0, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: true })).toBe(false);
  });

  it('is false for a never-invited veteran member (Inactive with no signal)', () => {
    expect(isCommitteeMemberAtRisk({ attended: 0, invited: 0, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: false })).toBe(false);
  });

  it('is true for an invited member who attended nothing (Inactive with negative signal)', () => {
    expect(isCommitteeMemberAtRisk({ attended: 0, invited: 5, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: false })).toBe(true);
  });

  it('is true for a Low-classified member', () => {
    expect(isCommitteeMemberAtRisk({ attended: 10, invited: 100, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: false })).toBe(true);
  });

  it('is false for Medium and High members', () => {
    expect(isCommitteeMemberAtRisk({ attended: 40, invited: 100, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: false })).toBe(false);
    expect(isCommitteeMemberAtRisk({ attended: 100, invited: 100, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: false })).toBe(false);
  });

  it('is never true for an Emeritus member, regardless of real attendance', () => {
    expect(isCommitteeMemberAtRisk({ attended: 0, invited: 20, votingStatus: EMERITUS, role: undefined, joinedWithinWindow: false })).toBe(false);
  });
  ...
  it('is still true for a non-staff Observer with a low real rate (the carve-out is role-based, not voting-status-based)', () => {
    expect(isCommitteeMemberAtRisk({ attended: 10, invited: 100, votingStatus: OBSERVER, role: undefined, joinedWithinWindow: false })).toBe(true);
  });
});

describe('isCommitteeMemberActive', () => {
  it('is true for any real attendance, even a Low-classified rate', () => {
    const input = { attended: 5, invited: 100, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: false };
  ...
  it('is true for a member who joined within the window, regardless of counts', () => {
    expect(isCommitteeMemberActive({ attended: 0, invited: 0, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: true })).toBe(true);
  });

  it('is false for a never-invited veteran member', () => {
    expect(isCommitteeMemberActive({ attended: 0, invited: 0, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: false })).toBe(false);
  });

  it('is false for an invited member who attended nothing', () => {
    expect(isCommitteeMemberActive({ attended: 0, invited: 5, votingStatus: VOTING_REP, role: undefined, joinedWithinWindow: false })).toBe(false);
  });

  it('is always false for Emeritus, even with real attendance or a fresh join', () => {
    expect(isCommitteeMemberActive({ attended: 5, invited: 5, votingStatus: EMERITUS, role: undefined, joinedWithinWindow: true })).toBe(false);
  });
