 ...
 *
 * Keyed by LFX slug — the same value the sidebar links with (`?project=<slug>`) and the one the
 * staging seed deliberately keeps production-faithful, so a mapping written here behaves the same
 * locally and on staging. Values are `newsletters_template_collections.slug` in Gatewaze; a
 * collection already carries its own `git_url`/`git_branch`, so naming the collection is what
 * selects the template repo — LFX never names a repo and cannot drift from it.
 *
 * `Object.create(null)` rather than a literal: a plain object would resolve inherited keys, so a
 * project slugged `toString` or `constructor` would return a function from the prototype chain
 ...
 * in Gatewaze's own data model and lands with multi-brand support after the pilot.
 */
export const GW_EMBED_TEMPLATE_COLLECTION_BY_SLUG: Readonly<Record<string, string>> = Object.assign(Object.create(null), {
  'agentic-ai-foundation': 'usercommunity',
});

/**
 * Collection used when nothing in scope names one.
 *
...
/** The scope a template is being resolved for. */
export interface GwEmbedTemplateScope {
  /** Slug of the project in view, or null/undefined on a foundation-level surface. */
  projectSlug?: string | null;
  /** Slug of the foundation in view. */
  foundationSlug?: string | null;
  /**
   * Whether `foundationSlug` is genuinely the parent of `projectSlug`.
   *
   * The host holds project and foundation in independent, separately-persisted slots, so the two
   * can describe unrelated scopes. The fallback only applies when the caller can vouch that they
   * are related — otherwise an unmapped project would inherit whichever foundation the user last
   * looked at, which is a cross-brand leak wearing a fallback's clothes.
   */
  foundationIsParentOfProject?: boolean;
}

/**
 * Resolves the template collection for a scope.
 *
 * Project first, then its foundation, then the default — a project may publish its own newsletter
 * with its own template, and inherits its foundation's when it does not.
 *
 * Slugs are matched exactly and own-property only. They come from LFX's project records, not user
 * input, and a near-miss is a mapping bug: it must fall through to the default rather than be
 * guessed into another brand's template.
 *
 * `map` is injectable purely so the precedence rule can be tested against two distinct mappings;
 * production always uses the constant.
 */
export function resolveGwEmbedTemplateCollection(
  scope: GwEmbedTemplateScope,
  map: Readonly<Record<string, string>> = GW_EMBED_TEMPLATE_COLLECTION_BY_SLUG
): string {
  const lookup = (slug?: string | null): string | undefined => (slug && Object.hasOwn(map, slug) ? map[slug] : undefined);

  const fromProject = lookup(scope.projectSlug);
  if (fromProject) {
    return fromProject;
  }

  // Only inherit from a foundation the caller has confirmed owns this project — or when there is
  // no project in scope at all, which is the foundation-level case.
  const foundationApplies = !scope.projectSlug || scope.foundationIsParentOfProject === true;

  return (foundationApplies ? lookup(scope.foundationSlug) : undefined) ?? GW_EMBED_DEFAULT_TEMPLATE_COLLECTION;
}
