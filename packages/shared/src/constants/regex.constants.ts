...
/** Generic Salesforce ID shape (no object-prefix constraint) — 15-char case-sensitive form, or its 18-char case-insensitive checksum-suffixed form. Matches the v1 platform's own `sfid.IsValid` gate (any object type: Contact, Lead, Account, custom objects). Unlike `SALESFORCE_ACCOUNT_ID_PATTERN`/`ORG_ACCOUNT_ID_PATTERN`, this doesn't check for a specific 3-char object prefix — use it where the caller only needs to know "is this SFID-shaped at all" (e.g. the LFXV2-1705 committee-member v1-mapping bridge, where the resolved identity could be a Contact SFID or another object type). Callers may also rely on this rejecting UUID-shaped values as a side effect of the `[A-Za-z0-9]`-only character class (no hyphens) — `member-v1-mapping.helper.ts`'s `parseMemberMappingResponse` uses exactly that to reject a "poisoned" pre-backfill mapping (LFXV2-2673) without a separate UUID check. Do not widen this class (e.g. to allow hyphens) without auditing that caller. */
