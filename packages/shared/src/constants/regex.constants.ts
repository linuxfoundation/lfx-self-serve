...
/** Generic Salesforce ID shape (no object-prefix constraint) — exactly 15 (case-sensitive form) or exactly 18 (its case-insensitive checksum-suffixed form) characters, anchored, no other length accepted. Matches the v1 platform's own `sfid.IsValid` gate (any object type: Contact, Lead, Account, custom objects). Unlike `SALESFORCE_ACCOUNT_ID_PATTERN`/`ORG_ACCOUNT_ID_PATTERN`, this doesn't check for a specific 3-char object prefix — use it where the caller only needs to know "is this SFID-shaped at all" (e.g. the LFXV2-1705 committee-member v1-mapping bridge, where the resolved identity could be a Contact SFID or another object type). Callers may also rely on this rejecting a 36-char UUID — `member-v1-mapping.helper.ts`'s `parseMemberMappingResponse` uses exactly that to reject a "poisoned" pre-backfill mapping (LFXV2-2673) without a separate UUID check. The exact 15-or-18 length is what does that rejecting (a UUID is 36 chars either way); the `[A-Za-z0-9]`-only character class rejecting the UUID's hyphens is true but incidental — do not widen the length bound (e.g. to `{15,36}`) without auditing that caller; widening only the character class (e.g. to allow hyphens) would NOT by itself let a UUID through. */
