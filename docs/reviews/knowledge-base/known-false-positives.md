...
**Why false:** under Angular Ivy, a static `class` attribute and a `[class]` property binding on the same element are **merged**, not clobbered. Verified empirically (GH-1784, 2026-08-22): rendering `<div class="static-a static-b" [class]="dyn">` in this repo's own TestBed on Angular 20.3.15 produced `class="static-a static-b dynamic-c"` — both static and bound classes present. That check is now a committed regression spec — `apps/lfx-one/src/app/shared/angular-class-binding-merge.spec.ts` — so a future Angular upgrade that ever changed this behavior fails CI instead of silently invalidating this entry. The repo already relies on this merge behavior in multiple places (e.g. `stat-card-grid.component.html`, `public-foundation-groups.component.html`, `public-project-groups.component.html`), so `[class]` (Angular's own recommended binding for a single dynamic class expression) is the established convention here, not a bug to "fix" by switching to `[ngClass]`.
