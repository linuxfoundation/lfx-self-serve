...
**Detect:** this applies only to a **tenant-private or membership-scoped** resource — one whose contract restricts it to callers entitled to that specific account/org/foundation/project. Establish that first: if the endpoint intentionally serves non-secret catalogue data to **every** authenticated user (the handler or its docs say so, as `org-identity.controller.ts` does for organization addresses — auth-gated but deliberately not membership-gated), it is **not** a finding. For a genuinely tenant-private resource, verify the entitlement is enforced **somewhere on the route's full chain before data access** — either the handler establishes the caller (`getEffectiveEmail(req)` / the bearer token, per `docs/reviews/backend-checklist.md`) and checks entitlement, **or an authorization middleware mounted ahead of the handler on that route already gates it** (for example `requireOrgLensAccess` on the `/:orgUid/lens` family in `orgs.route.ts`, whose controllers deliberately carry no handler-local check because the middleware runs first). Trace route → middleware → handler; a preceding gate satisfies this. Flag it Critical only when the identifier flows straight from the request into the data lookup with **no** entitlement check anywhere on the chain — a membership/ownership lookup, an upstream call made **as the user**, a route-level access middleware, or an equivalent. This is orthogonal to route classification: a route can be correctly required-auth and still fail this.
