...
**Read when:** `app.config.ts`, anything under `app/shared/guards/` or `app/shared/interceptors/`, any `*.routes.ts`, any added or changed file under `apps/lfx-one/src/server/routes/` (`*.route.ts` included, not only new files), `apps/lfx-one/src/server/server.ts`, any file under `apps/lfx-one/src/server/middleware/` (auth, and authorization/access-control middleware such as `require-*-access.middleware.ts`), any auth/access-control helper under `apps/lfx-one/src/server/helpers/` (e.g. `org-lens-read-access.helper.ts`, where a `require-*-access` middleware delegates its actual entitlement logic), or any file under `apps/lfx-one/src/server/controllers/` or `apps/lfx-one/src/server/services/`. Cross-checked in Steps 3-4 of the learnings-review playbook (KB-match gate in Step 3, false-positive filter in Step 4).
...
**Detect:** the trigger is any added or changed **route**, **and separately any change to the auth infrastructure itself** — the `app.use(authMiddleware)` mount line in `server.ts` (moved or removed), the terminal catch-all row of `DEFAULT_ROUTE_CONFIG` (`{ pattern: '/', auth: 'required' }`, which `classifyRoute` returns for every well-formed path), or the classifier logic in `auth.middleware.ts` — because those expose **existing** routes with no route line changing. For each, resolve the affected path's **effective path**, its **resulting auth classification** against `DEFAULT_ROUTE_CONFIG`, **and where it is mounted relative to `app.use(authMiddleware)` in `server.ts`**. Flag it Critical when a sensitive route becomes anonymously reachable through any of: (a) a new **or modified** `DEFAULT_ROUTE_CONFIG` entry that lands the path in `auth: 'optional'`/`'public'` — including editing an existing entry from `required`, widening an existing entry's pattern to swallow more paths, or reordering an `optional`/`public` entry ahead of a more restrictive one so it wins the match (compare the affected path's effective classification **before and after** the change, since no route handler need change for this); (b) a new `/public/api/*` (or other public-prefix) mount in `server.ts`; (c) a new or changed handler added to a router **already mounted** under an existing optional/public prefix (any `public-*.route.ts` under `/public/api`, or an SSR path matching an existing optional pattern), which inherits that classification with no config or mount change; (d) a handler **mounted before `app.use(authMiddleware)`** in `server.ts` — the global middleware never runs for it, so it is anonymous regardless of what `DEFAULT_ROUTE_CONFIG` would classify its path as (the sitemap/static handlers already sit here deliberately; a sensitive route added above that line is the defect); or (e) the auth mount itself moving down past existing handlers, the terminal `{ pattern: '/', auth: 'required' }` catch-all row of `DEFAULT_ROUTE_CONFIG` being weakened or removed (this is what `classifyRoute` returns for every well-formed path — **not** `DEFAULT_CONFIG.defaultAuth`, which is only reached on a decode-failure/malformed path, so weakening `defaultAuth` alone does **not** reclassify existing handlers and is not by itself a finding), or a classifier change that reclassifies existing paths into the optional/public lane — an infrastructure-only regression that exposes routes already present. (`DEFAULT_CONFIG.defaultType` is **not** in scope here: it only selects the fallback response shape, `api` vs `ssr`, and never affects anonymous reachability.) Do **not** flag a normal new `/api/*` mount placed **after** the global middleware for lacking an inline `authMiddleware` — the global mount already covers it, and demanding `app.use('/api/<prefix>', authMiddleware, router)` or `router.use(authMiddleware)` is a false positive in this codebase.
...
**Pattern:** a route reads a resource identifier from the request (`req.params`/`req.query`/body) and, **on any authenticated execution path**, returns that resource's data without checking that the authenticated caller is allowed to access **that specific** identifier. This is not limited to `required`-auth routes: an `optional`-auth handler that returns a safe anonymous response but serves tenant-scoped data **when a session is present** has the same gap on its authenticated branch, and the public-exposure pattern would not fire because anonymous callers receive no protected data. Authentication is satisfied — the caller has a valid session — but authorization is not: any logged-in user can substitute another tenant's id and read across the account boundary (IDOR / broken object-level authorization). The global `authMiddleware` does **not** close this gap; it only proves the caller is authenticated, never that they own the requested object.

**Detect:** this applies only to a **tenant-private or membership-scoped** resource — one whose contract restricts it to callers entitled to that specific account/org/foundation/project. Establish that first: if the endpoint intentionally serves non-secret catalogue data to **every** authenticated user (the handler or its docs say so, as `org-identity.controller.ts` does for organization addresses — auth-gated but deliberately not membership-gated), it is **not** a finding. For a genuinely tenant-private resource, evaluate **every authenticated execution path** (a `required` route, or the session-present branch of an `optional` route) and verify the entitlement is enforced **somewhere on the route's full chain before data access** — either the handler establishes the caller (`getEffectiveEmail(req)` / the bearer token, per `docs/reviews/backend-checklist.md`) and checks entitlement, **or an authorization middleware mounted ahead of the handler on that route already gates it** (for example `requireOrgLensAccess` on the `/:orgUid/lens` family in `orgs.route.ts`, whose controllers deliberately carry no handler-local check because the middleware runs first). Trace route → middleware → handler; a preceding gate satisfies this. An upstream call made **as the user** counts **only when that upstream actually authorizes the supplied identifier** (an access/FGA check on that id) — forwarding the user token to an auth-only or non-authorizing upstream does **not** close the gap, since the token proves who the caller is, not that they may read this specific resource. Flag it Critical when the identifier flows straight from the request into the data lookup on an authenticated path with **no** such entitlement check anywhere on the chain. This is orthogonal to route classification: a route can be correctly required-auth (or optional-auth) and still fail this.
...
**Failure message:** Authenticated route (required, or the session-present branch of an optional route) queries a caller-supplied id without an ownership/entitlement check — cross-account (IDOR) access.
