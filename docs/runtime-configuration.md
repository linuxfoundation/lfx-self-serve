...
| Variable                | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Example              |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| `VALKEY_URL`            | Shared Valkey connection URL. Requires a TLS-secured (`rediss://`) URL in production — the app refuses to start otherwise, since AuthStateService (#1938) and, when enabled, the session store write sensitive identifiers/tokens to it. Unset disables caching (direct-fetch fallback) for most features — but weekly-brief AI action-item extraction (LFXV2-3043) skips extraction entirely rather than direct-fetching, since the cache is the only bound on AI proxy spend for that endpoint; unset `VALKEY_URL` means that feature always returns zero items. The per-user meeting-invite lock (LFXV2 #2241) also depends on Valkey being reachable for cross-replica protection — when it's unset, or enabled but temporarily unreachable, that lock degrades to a per-replica in-memory mutex until Valkey is set/recovers. Flow C's CSRF state nonce (`AuthStateService`, #1938) also depends on this — unset, it falls back to storing the nonce on `req.appSession`, which is exposed to the concurrent session-write race the Valkey-backed record otherwise avoids. | `rediss://host:6379` |
