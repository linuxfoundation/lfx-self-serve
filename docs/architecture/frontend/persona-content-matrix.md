...
| **Communications** section |                           | `canSeeNewsletters()` — ED **or** `canWrite()`                                                                                                                                                                        |
| → Newsletters              | `/project/newsletters`    | `canSeeNewsletters()`; retargets to `/project/gw/newsletters` (the embedded Gatewaze module) while the pilot flag is on **and** the selected foundation or project is on the tenant allowlist, same guards either way |
