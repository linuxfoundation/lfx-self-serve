...
| `orgLensClaM3EnabledGuard`   | `/org/{segment}/easycla` (and legacy `/org/easycla`) and children (CanMatch)                                             | Browser: waits for LaunchDarkly READY, then `ORG_LENS_CLA_M3_ENABLED_FLAG`; fail closed, redirecting to the addressed organization's overview (`/org/{segment}/overview`; on the legacy address, the selected organization's — `/org/overview` while nothing is selected). SSR: always returns `true` — enforcement defers to browser after hydration                                                                                                                                                                                                                                                |
| `orgSegmentMatchGuard`       | `/org/:orgSegment` (CanMatch)                                                                                            | Matches only values shaped like an organization segment — a slug (matched case-insensitively; `orgPathParamGuard` then canonicalizes the address to lowercase) that is not an Org Lens page name, or an 18-char SFID; anything else falls through to the in-shell 404 instead of a resolver round trip.                                                                                                                                                                                                                                                                                              |
