...
| → Broadcasts               | `/foundation/gw/broadcasts`    | Pilot flag + tenant allowlist only — LFX has no broadcasts page of its own, so the entry is absent otherwise. Same access gate as Newsletters                                                                 |
...
| → Broadcasts               | `/project/gw/broadcasts`  | Pilot flag + tenant allowlist only — LFX has no broadcasts page of its own, so the entry is absent otherwise. Same access gate as Newsletters                                                                                                                                                                                                                                                                                                                |
