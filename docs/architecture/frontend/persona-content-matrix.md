...
| **Communications** section |                           | `canSeeNewsletters()` — ED **or** `canWrite()`; **any persona** while `gatewaze-embed-enabled` is on (pilot mock)       |
| → Newsletters              | `/project/newsletters`    | `canSeeNewsletters()`; retargets to `/project/gw/newsletters` (the embedded Gatewaze module) while the pilot flag is on |
| → Broadcasts               | `/project/gw/broadcasts`  | Pilot flag only — LFX has no broadcasts page of its own, so the entry is absent when the flag is off                    |
...
| `gatewazeEmbedEnabledGuard`  | `/foundation/gw/**`, `/project/gw/**` (CanMatch)                                        | Browser: waits for LaunchDarkly READY, then `GATEWAZE_EMBED_ENABLED_FLAG`; fail closed to `/`. Paired with the `LFX_GATEWAZE_EMBED_ENABLED` server flag, which independently gates the `/api/gw` proxy — the client flag hides the UI, the server flag closes the BFF |
