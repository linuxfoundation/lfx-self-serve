...
| `orgPathParamGuard`          | `/org/:orgSegment/**` (CanActivate)                                                                                      | Resolves the segment through the FGA-filtered BFF (`GET /api/orgs/resolve/:segment`) and adopts the organization; canonicalizes the address to the slug (or SFID). Browser: a 404/409 lands on `/org/not-found` without touching the current selection (FR-022–FR-024). SSR: issues no redirect — clears the selection and returns `true` so the server renders the page skeleton (FR-021); the browser run decides after hydration.                                                                                                                                   |
