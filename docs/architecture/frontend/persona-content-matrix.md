...
| **Communications** section |                           | `canSeeNewsletters()` — ED **or** `canWrite()`                                                                                                  |
| → Newsletters              | `/project/newsletters`    | `canSeeNewsletters()`; retargets to `/project/gw/newsletters` (the embedded Gatewaze module) while the pilot flag is on, same guards either way |
| → Broadcasts               | `/project/gw/broadcasts`  | Pilot flag only — LFX has no broadcasts page of its own, so the entry is absent when the flag is off. Same access gate as Newsletters           |
