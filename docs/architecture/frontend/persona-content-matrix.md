...
| **Meeting service** | `POST /itx/meetings` and `POST /itx/past_meetings`: OpenFGA `meetings_creator` on `project:{project_uid}` **or** `writer` on `committee:{committees[0].uid}` when the body includes committees ([LFXV2-2395](https://linuxfoundation.atlassian.net/browse/LFXV2-2395)). Update/delete/registrant routes unchanged (`organizer` on `v1_meeting`). |
