...
| `orgPathParamGuard`          | `/org/:orgSegment/**` (CanActivate)                                                                                      | Resolves the segment through the FGA-filtered BFF (`GET /api/orgs/resolve/:segment`) and adopts the organization; canonicalizes the address to the slug (or SFID); a 404/409 lands on `/org/not-found` without touching the current selection (FR-022–FR-024).                                                                                                                                                                                                                                                                                                         |
| `orgLensRoiEnabledGuard`     | `/org/{segment}/roi` and children (CanMatch)                                                                             | Browser: waits for LaunchDarkly READY, then `ORG_LENS_ROI_ENABLED_FLAG`; fail closed, redirecting to the **addressed** organization's overview via `orgLensPagePath` (recognition-time — the URL, not the selection). SSR: always returns `true`.                                                                                                                                                                                                                                                                                                                      |
