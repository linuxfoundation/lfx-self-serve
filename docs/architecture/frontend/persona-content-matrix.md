...
| `orgPathParamGuard`          | `/org/:orgSegment/**` (CanActivate)                                                                                      | Resolves the segment through the FGA-filtered BFF (`GET /api/orgs/resolve/:segment`) and adopts the organization; adopts the organization on a hit (browser and SSR alike). Browser: canonicalizes the address to the slug (or SFID); a 404/409 lands on `/org/not-found` without touching the current selection (FR-022–FR-024). SSR: no redirect and no canonicalization — a hit renders the addressed organization; where the browser would redirect, the server clears the selection and returns `true` so the page skeleton renders (FR-021) and the browser run decides after hydration. |
