...
| `orgLensClaM3EnabledGuard`   | `/org/easycla` and its children (CanMatch)                                              | Browser: waits for LaunchDarkly READY, then `ORG_LENS_CLA_M3_ENABLED_FLAG`; fail closed, redirecting to `/org/overview`. SSR: always returns `true` — enforcement defers to browser after hydration. Client half only; `ServerFeatureFlag.OrgLensClaM3` gates the BFF |
