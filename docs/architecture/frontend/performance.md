...
3. **Font / icon loading**: Google Fonts are a `<link>` in `index.html` with `preconnect` to `fonts.gstatic.com` and `display=swap`. Production inlines that CSS (Angular `fonts.inline`), so there is no `fonts.googleapis.com` preconnect. Do not `@import` them from `styles.scss` — that cannot start until the main stylesheet has downloaded. The Font Awesome kit is `defer`red so it is not parser-blocking.
4. **Invite landing bootstrap**: `/invite` and `/invite/error` skip LaunchDarkly entirely (no client; flags stay at code defaults), Intercom boot, Segment/Plausible, persona/profile refetch, feature-module preload, and the product header. The signed-in user's Intercom identity is still staged on `IntercomService` (never while impersonating — the impersonation override leaves the Intercom JWT claim as the operator's), so the error page's "Contact support" boots the widget on click as that user. DataDog RUM still runs; invite `token` query params are redacted before send. Leaving those pages is a full navigation so the next load runs a normal bootstrap.
