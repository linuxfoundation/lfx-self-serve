...
- **Instrumented readiness wait**: `waitForReady()` centralizes the guard-facing timeout so every flag-gated route — both `CanMatch` guards and the `CanActivateFn` guards (`campaignAccessGuard`, `marketingImpactAccessGuard`, `formationOverviewRedirectGuard`) — reports the same way to RUM on failure; the two formation guards (`formationProjectEnabledGuard`, `formationOverviewRedirectGuard`) reach it through the shared `resolveFormationFlag` in `formation-checklist-gate.util.ts`, instead of each guard duplicating its own wait/timeout/log logic
