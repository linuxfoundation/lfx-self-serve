...
- **Instrumented readiness wait**: `waitForReady()` centralizes the guard-facing timeout so every flag-gated route that goes through it — the `CanMatch` guards and the `CanActivateFn` guards (`campaignAccessGuard`, `marketingImpactAccessGuard`, `formationOverviewRedirectGuard`) — reports the same way to RUM on failure; the two formation guards (`formationProjectEnabledGuard`, `formationOverviewRedirectGuard`) reach it through the shared `resolveFormationFlag` in `formation-checklist-gate.util.ts`, instead of each guard duplicating its own wait/timeout/log logic. Two budgets exist: the shared default `FEATURE_FLAG_READY_TIMEOUT_MS` (10 s), and `FEATURE_FLAG_REDIRECT_READY_TIMEOUT_MS` (3 s) for `formationOverviewRedirectGuard` alone, which only redirects a page that is already rendered and fails **open** when the budget runs out — recording that decision on `ProjectContextService.formationOverviewAllowedSlug` so `SidebarNavService` keeps the full nav in step with the dashboard it admitted instead of collapsing it once the provider catches up. `formationEnabledGuard` (the foundation-lens formations queue) predates `waitForReady` and still carries its own inline 5 s wait, so its timeouts are not reported through this path
...
`initializationTimeout` is not split per environment — both prod and dev derive it from the single shared `FEATURE_FLAG_READY_TIMEOUT_MS` constant (10s), so the bootstrap wait and every guard's default `waitForReady()` wait move together. The one guard-level exception is `formationOverviewRedirectGuard`, which passes the shorter `FEATURE_FLAG_REDIRECT_READY_TIMEOUT_MS` (3s) because the page under it is already rendered and it fails open on timeout. Only the logger level differs by environment:
