...
Org Lens is the one lens whose context **is** in the route: pages are addressed `/org/{segment}/{page}` (spec 050), where `{segment}` is the organization's slug, or its 18-char SFID when it has none. The legacy `/org/{page}` form still routes and is redirected to the selected organization's address. Two rules follow from that, and both are enforced only by convention — read them before adding an Org Lens page or link:
