...
Rule 1 before 2–3 is the #2216 fix (a present grant is authoritative whatever else failed); rule 4 before 5–6 is the #2533 never-fall-through rule (a failed check must not read as the contractor or employee copy). Rules 2–4 live in one place, `OrgLensEmptyStateService.classifyLookup(holdsAnything)`, which both the page and the dead end call. Every Org page and the dead end also leave their skeleton on one rule, `OrgLensEmptyStateService.pageReady`: `settled` (which includes an owed contractor read check), plus the caller's org list when they have switcher access — a caller without it never starts that list.
