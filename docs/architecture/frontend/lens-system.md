...
Re-addressing after a selection change is `OrgLensNavigationService.navigateToSelectedOrg(intent)`. It is a no-op unless the selection carries a URL segment. A `'switch'` (the viewer picked an organization in the selector) re-addresses any Org Lens page — including off the `/org/not-found` dead end — and **pushes** history so Back returns to the pre-switch organization; except when the address named no organization (bare `/org`, legacy `/org/{page}`), where there is nothing to go back to and the insert **replaces** like a canonicalization. A `'default'` (the app selected an organization because none was, or restored the cookie's) only fills an organization into a legacy address that names none, **replaces** history, and never leaves `/org/not-found` or overrides an address that already names an organization. It reads the router's current address, so if a navigation is in flight when the org list answers (a link clicked during bootstrap) the write is deferred until the router is idle — through any cancel-and-redirect — and then re-reads the destination.

The slug in every Org Lens address and link is the **index's** (the org-items row or the resolver's answer), never member-service's. Addresses are resolved against the index (`/api/orgs/resolve/:segment` reads query-service), and during index lag the canonical record (`GET /api/orgs/uid/:uid`, member-service) runs ahead of it — a slug taken from there would reopen as not-found for anyone the address is shared with. So `AccountContextService.applyCanonicalRecord` never touches the slug — not to set it into a gap, not to overwrite an indexed value, and not even to unset one it disagrees with (a rename in flight): the next navigation asks the resolver, which is the index and hands the indexed slug straight back, so an unset would only flip-flop. A selection with no indexed slug yet addresses as its SFID until an indexed row answers (org list on bootstrap, resolver on the next navigation); a renamed slug reaches links first (every `orgLensLink` re-renders from the selection the moment an indexed row carries it) and the address bar on the next navigation the path guard resolves — a `'default'` write never re-addresses an already-addressed page, so links and address can briefly differ, and the address is the one that lags. The one caller that knows the client is ahead of the index — the Org Profile edit hook (`updateCanonicalRecord`, spec 021) — forgets the held slug when the rename changed it, without adopting the new one: the old slug would otherwise be trusted by the guard's shortcut for the rest of the session and stop resolving the moment the index catches up, so links and address fall to the SFID until the resolver answers with the indexed slug (old until reindexed, then new — converging, since the resolver is the index). An address-adopted selection is never re-checked by the org list; its slug comes from the resolver alone. On the guard side, `orgPathParamGuard`'s no-round-trip shortcut trusts a held slug only when it is slug-shaped (`isOrgSlugSegment`), so an SFID-shaped value can never pass another organization's SFID address off as the selection. Nothing re-writes an address behind a canonical-record fetch (`orgPathParamGuard` still canonicalizes a deep link from the resolver's own answer, and an Org Profile rename reaches the address only once the index has it). A rename reaches links and addresses together when the index has caught up and the org list is next loaded.
