...
5. an LF contractor (`isContractor`) the server refuses → `contractor-no-grant` (#2961). With an organization selected (persona seed, cookie) the page asks the read gate, `GET /api/orgs/:orgUid/lens/read-check` (204 admitted, 403 `FORBIDDEN` refused; any other failure is an outage, not a refusal), and shows a skeleton until it answers; with nothing selected it means the contractor holds nothing. It keys on the server's answer, never the roster: an organization the gate admits renders the page even when no roster row or persona seed lists it, so FGA-only readers such as key-contact auditors keep their page;
6. the caller holds nothing → `no-organization`;
7. otherwise a selection is pending → render the page.

Rule 1 before 2–3 is the #2216 fix (a present grant is authoritative whatever else failed); rule 4 before 5–6 is the #2533 never-fall-through rule (a failed check must not read as the contractor or employee copy). Rules 2–4 live in one place, `OrgLensEmptyStateService.classifyLookup(holdsAnything)`, which both the page and the dead end call. Every Org page renders the page-level state (#2961). The pages that wait for the default selection from the org list (overview, projects, ROI and its project detail, meetings, groups, EasyCLA) and the dead end leave their skeleton on `OrgLensEmptyStateService.pageReady`: `settled` (which includes an owed contractor read check), plus the caller's org list when they have switcher access — a caller without it never starts that list. The other Org pages (people, contributions, training, memberships and detail, profile, project detail, events) never waited for the list and gate on `settled`. Until its gate opens, every page shows its skeleton, never a blank area or a state. A page header that names the organization shows the name only once the content renders, never beside a page-level state.

`/org/not-found` (`org-not-found.component.ts`) applies, in order: **`not-found-staff`** when the caller is LF team (the rule-1 analogue — their own list is empty by design and their way in is switcher search); then **`classifyLookup`** (rules 2–4) with "holds anything" = the caller's grant sets or list rows are non-empty (unfiltered — an open switcher search must not flip the state); then **`could-not-load`** when the caller's own list failed to load (`OrgNavigationService.upstreamFailed`); then **`contractor-no-grant`** when an LF contractor holds nothing (#2961); then **`wrong-organization`** when the caller holds something (the list is the primary control — picking a row leaves through `OrgLensNavigationService.navigateToSelectedOrg('switch')`, or the legacy `/org/overview` when the row has no URL segment) or **`no-access`**. The `no-access` / `wrong-organization` states never name the addressed organization (spec 050 DR-002).
...
**Wire contract** — `GET /api/orgs/me/role-grants` (`RoleGrantsResponse`, `packages/shared/src/interfaces/org-selector.interface.ts`) carries, besides the grant sets: `lookupOutcome` (`ok` | `partial` | `failed`), `staffCheck` (`ok` | `failed`), `isContractor` (#2961: `team:lf-contractor` member and not LF team, resolved in the same batched team check, fail-closed `false`; it explains an empty Org Lens and grants nothing) and, only when the staff check failed, `correlationId` — also sent as the `X-Correlation-Id` response header (its own header, not `X-Request-Id`: it is the id logged by the computation that produced the result, which within the short cache window may be an earlier request's). The server (`apps/lfx-one/src/server/services/org-role-grants.service.ts`) never caches a failed roster; a failed staff check is cached only for `ORG_ACCESS_AWARE_FAILED_STAFF_CHECK_CACHE_TTL_MS` (5 s) with its id — the short entry shields the gate reads (every section request, the org-items list) during an authorizer outage while recovery is not pinned for the full TTL; a degraded roll-up keeps the full TTL (its causes are often structural). The viewer's Retry (`?refresh=1`) recomputes once per click, coalesced per burst, and rewrites the entry — but only past a degraded or failed entry; a clean `ok` entry is never bypassable, so the flag is not a fan-out lever.
