...
Every writer — the selector (switch and not-found escape) and the org-list bootstrap — then chains `reconcileAddress()` behind the canonical-record fetch. If that record carries a different slug than the indexed row the address was written with (index lag, a rename, a slug removed), the written segment is **replaced** — under four bounds, all to the write this service itself made:
