...
Re-addressing after a selection change is `OrgLensNavigationService.navigateToSelectedOrg(intent)`. It is a no-op unless the selection carries both a URL segment and a uid (the key `reconcileAddress` binds to). A `'switch'` (the viewer picked an organization in the selector) re-addresses any Org Lens page — including off the `/org/not-found` dead end — and **pushes** history so Back returns to the pre-switch organization. A `'default'` (the app selected an organization because none was, or restored the cookie's) only fills an organization into a legacy address that names none, **replaces** history, and never leaves `/org/not-found` or overrides an address that already names an organization.

Both callers then chain `reconcileAddress()` behind the canonical-record fetch. If that record carries a different slug than the indexed row the address was written with (index lag, a rename, a slug removed), the written segment is **replaced** — under four bounds, all to the write this service itself made:

- only after that write's navigation has settled (`Router.url` moves on activation, and the canonical fetch can win the race);
- only if no newer write superseded it (a re-pick of the same organization included);
- only for the same organization (a later default of another organization must not re-address a page a switch wrote);
- only while the live address still carries the written segment (a redirect under the same organization, such as a flag guard sending `/roi` to `/overview`, keeps it and is still reconciled).

Deep links do not go through it: `orgPathParamGuard` canonicalizes from the resolver's own answer.
