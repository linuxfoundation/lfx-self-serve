...
The segment written is the selection's at the time of the write — the indexed org row's slug. It is deliberately **not** re-written when the canonical record (member-service, `GET /api/orgs/uid/:uid`) later arrives with a different slug: addresses are resolved against the index (`/api/orgs/resolve/:segment` reads query-service), and during index lag the canonical slug is the one the resolver cannot answer yet — an address carrying it would reopen as not-found for anyone it is shared with. The indexed slug is the copyable one; a rename reaches addresses when the index has caught up and the org list is next loaded. Deep links likewise keep the resolver's own answer.
