...
Org Lens is the one lens whose context **is** in the route: pages are addressed `/org/{segment}/{page}` (spec 050), where `{segment}` is the organization's slug, or its 18-char SFID when it has none. The legacy `/org/{page}` form still routes and renders the selected organization; the address is rewritten to the organization's form only when the selection changes under it — an organization switch, or the automatic default picked when nothing was selected — plus one escape: on the `/org/not-found` dead end, picking the organization that is already selected leaves for its overview; and, after a switch or default write (not the escape — its selection is already canonical), once when the selection's canonical record arrives with a different slug than the indexed row's (`reconcileAddress`, below). The general legacy-address redirect is spec 050 US3, not yet built — see `app.routes.ts`. Three rules follow from that, and all are enforced only by convention — read them before adding an Org Lens page or link:
...
Re-addressing after a selection change is `OrgLensNavigationService.navigateToSelectedOrg(intent)` — a no-op unless the selection carries both a URL segment and a uid (the key `reconcileAddress` binds to): a `'switch'` (the viewer picked an organization in the selector) re-addresses any Org Lens page — including off the `/org/not-found` dead end — and **pushes** history so Back returns to the pre-switch organization; a `'default'` (the app selected an organization because none was) only fills an organization into a legacy address that names none, **replaces** history, and never leaves `/org/not-found` or overrides an address that already names an organization. Both callers then chain `reconcileAddress()` behind the canonical-record fetch: if that record carries a different slug than the indexed row the address was written with (index lag, a rename), the segment is **replaced** — but only after the write's navigation has settled, only if no newer write superseded it, only for the same organization, and only while the live address still carries the written segment (a redirect under the same organization keeps it and is still reconciled). Deep links do not go through it: `orgPathParamGuard` canonicalizes from the resolver's own answer.
