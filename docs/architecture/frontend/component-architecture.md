...
For the scroll-spy used inside a Level 2 page, follow `account-settings.component.ts`: observe heading **sentinels** rather than whole sections (two whole sections light at once mid-scroll), keep an `intersecting` Set so exactly one item is ever active, add an end sentinel for a short last section, and register teardown once via `destroyRef.onDestroy` — not inside the setup function, which re-runs whenever the sticky offset changes.
