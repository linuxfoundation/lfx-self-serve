...
A Level 2 tab composes `HealthMetricsL2ShellComponent` (`modules/dashboards/components/health-metrics-l2-shell/`) rather than re-implementing the layout. The tab passes its section list, id prefix and sub-nav items, projects each section body through an `<ng-template lfxHealthMetricsL2Section="key">`, and relays each body's `reading`/`settled` to the shell. The shell owns the sub-nav, the bounded scrolling pane, the scroll-spy and fragment deep links; a section with no projected body renders an anchored "Awaiting data" placeholder.
