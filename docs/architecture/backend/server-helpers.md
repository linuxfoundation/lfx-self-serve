...
| `formation-mapper.helper.ts` | Maps upstream `lfx-v2-formation-service` shapes (checklist, items) onto the BFF's `@lfx-one/shared` formation types — `sub_stage` normalization (GH-2366, GH-2328) instead lives in `packages/shared/src/utils/formation.utils.ts`'s `normalizeFormationSubStage`/`getFormationQueueStageDisplay`, called from both `formation.service.ts` (queue rows) and this file's `mapUpstreamFormationChecklist` (checklist reads, sourced from the project record's `stage` since the upstream checklist payload carries no stage field of its own) |
