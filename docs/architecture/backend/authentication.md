...
| An indefinitely-retained log line recording who took an action                        | **sub**      |

> **Default to `username`.** `sub` has been phased out of backend identity references and no upstream currently requires it — see the migration note below. The one deliberate exception is structured **server log** metadata meant to persist indefinitely (distinct from the client-side RUM/OpenFeature identity row above, which stays `username`): `username` is a human-readable LFID handle, and putting it in a durably-retained log stream is exactly the PII exposure `docs/reviews/knowledge-base/security.md`'s `security/pii-in-logs-and-identifiers` warns against (see `weekly-brief.service.ts`'s `rating_recorded`/`share_weekly_brief_slack_sent` log calls). This carve-out is scoped to backend log metadata specifically — it does not extend to upstream calls, cache keys, persisted records, or client-side analytics identity, all of which still use `username`.
...
| --------------------------- | ------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `getEffectiveUsername(req)` | Impersonated username or OIDC nickname/username/preferred_username | **Preferred** for all new identity references, including upstream calls, cache keys, and persisted records                                                                                                                          |
...
- **`getEffectiveSub`** is annotated `@deprecated` in `auth-helper.ts` for identity references generally. Its remaining call sites fall into three kinds, not exhaustively listed: incidental auth-service/lookup holdovers that are cleanup candidates (e.g. badges' and CLA's identity/email lookups, three of `profile.controller.ts`'s four call sites, mktg-agents' internal session-token binding — none require the prefixed `sub` upstream); one response-payload field that's a genuine `getEffectiveUsername` migration candidate rather than a holdover (`profile.controller.ts`'s `UserProfile.id`, which the carve-out below explicitly does not cover); and a deliberate use in indefinitely-retained server log metadata (`weekly-brief.service.ts`), which is not a cleanup candidate — see the log-line carve-out in "When to use which" above.

When adding new code, use `username` — unless the value is going into an indefinitely-retained log line, in which case use `sub` and note why inline (mirror the existing `rating_recorded`/`share_weekly_brief_slack_sent` comments in `weekly-brief.service.ts`). No upstream currently requires the prefixed `sub` outside that log-metadata case.
...
`AuthStateService` (`apps/lfx-one/src/server/services/auth-state.service.ts`) sidesteps this by keying the CSRF nonce in its own short-TTL Valkey record (`auth-state:v1:<nonce>`, 600s TTL) instead of `req.appSession`, independent of `SESSION_STORE_ENABLED`. The callback consumes (looks up and deletes) the record by nonce and requires it belong to the authenticated user, rather than comparing against the session. When `VALKEY_URL` is unset, `AuthStateService` falls back to session-based storage so Flow C keeps working — that fallback retains the original race, but (as of PR #2604) it also persists the issuing sub as `profileAuthSub` and re-validates it at consume time, so the same-sub CSRF check holds even without Valkey.

**Rollout note:** this nonce cutover is not compatible across a live rolling deploy or rollback — a pod running the old code cannot read a nonce written to Valkey by a new pod, and a new pod treats an old pod's session-only nonce as an authoritative Valkey miss. Either window forces the affected login to restart. Deploying this change requires a no-overlap rollout (and rollback plan) for Flow C traffic; it does not have cross-version compatibility built in.
