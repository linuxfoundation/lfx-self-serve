...
- **`getEffectiveSub`** is annotated `@deprecated` in `auth-helper.ts` for identity references generally. Its remaining call sites split into two kinds: incidental holdovers that are cleanup candidates (e.g. badges' and CLA's auth-service email/identity lookups, mktg-agents' internal session-token binding — none require the prefixed `sub` upstream), and a deliberate use in indefinitely-retained server log metadata (`weekly-brief.service.ts`), which is not a cleanup candidate — see the log-line carve-out in "When to use which" above.
