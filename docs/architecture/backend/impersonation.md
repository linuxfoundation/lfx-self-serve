...
| `getEffectiveSub(req)`      | Impersonated sub or OIDC sub                                       | **`@deprecated`** for identity references generally — Auth0 sub (prefixed, e.g. <code>auth0&#124;jdoe</code>); a handful of illustrative (not exhaustive) callers remain, including incidental auth-service lookups (badges, CLA, mktg-agents session binding) and a deliberate use for indefinitely-retained server log metadata (weekly-brief share/rating events) — see [`authentication.md`](./authentication.md#-identity-claims-username-vs-sub) for the full breakdown |
