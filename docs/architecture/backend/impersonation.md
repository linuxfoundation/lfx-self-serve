...
4. When an active impersonation session exists, overrides `auth.user` with the target user's claims (sub, email, username, SSO username claim, preferred_username, name, nickname, picture) and sets `auth.impersonating = true` + `auth.impersonator` — every claim `FeatureFlagService`'s `targetingKey` chain reads must be overwritten here, or a stale value from the impersonator's own session wins the fallback (LFXV2 #2316). `given_name`/`family_name` (and their `first_name`/`last_name` alternates, read by some consumers as a fallback) are blanked rather than overwritten, since the impersonation session only stores the target's combined display name, not a first/last split — leaving them stale would otherwise leak the impersonator's real name into forms that pre-fill from those fields
