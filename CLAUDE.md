...
1. **Wait for every running review to complete.** Each trio audits one commit, so the trio invoked by every recent commit needs to have returned before you continue. A report that leads with `INCOMPLETE` does NOT count as returned clean — relaunch that child (in the final-commit path, rerun the whole sweep batch) before continuing.
