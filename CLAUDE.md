...
2. **Pin one complete range when no batch is active.** Track a coverage marker called the **last successfully reviewed target**. Before the first batch succeeds, initialize this marker to that batch's `base_sha` — the commit immediately before the earliest unreviewed commit — so an invalid first batch leaves a defined coalescing boundary. A valid batch advances the marker to its `target_sha` even when it reports findings; an invalid batch leaves the marker unchanged. For a normal batch, pin `target_sha=$(git rev-parse HEAD)`, `base_sha=$(git rev-parse HEAD^)`, and label it `the latest commit`. If commits accumulated during an active trio, drain it, then pin `base_sha` to the marker, `target_sha` to current `HEAD`, and label the range `the commits since the last review`. Record both full SHAs — shell variables do not survive between tool calls.
...
4. **Canonical child prompt (exact for post-commit batches and full-branch sweeps):**
   ...
   ```text
   Load exactly one skill and follow it end to end as your complete review playbook: <skill>. Do not load any other review skill. For a repo-owned skill, the target blob from git show <target_sha>:.claude/skills/<physical-skill>/SKILL.md is authoritative. A named Skill-tool load may be used only when its content can be shown equivalent to that blob; otherwise follow the pinned blob and report that source. If the pinned blob cannot be read, return "INCOMPLETE — could not load <skill>" instead of reviewing without it. Never use a moving working-tree skill fallback. If lfx-skills:lfx-general-code-review is unavailable, return "INCOMPLETE — lfx-skills:lfx-general-code-review unavailable" instead of reviewing unguided; this installed central dependency is outside the target repo and has no target blob. Report only: do not edit tracked files, stage, commit, push, or post anything to GitHub — the parent session applies every fix.

   The repo root below is authoritative: run all git commands there and skip the loaded skill's repo-location search. The pinned range below is authoritative: audit target_sha against base_sha exactly, even if HEAD or origin/main moves after launch. Wherever the loaded skill names git show, HEAD, or origin/main...HEAD as its diff range, use git diff <base_sha> <target_sha> instead. Discover target-repo paths from the target tree. Every target-repo review substrate required by the loaded skill must come from target_sha with git show, including its repo-owned skill body, CLAUDE.md, rules, required review checklists, conditionally routed architecture and contract docs, and selected docs/reviews/knowledge-base/** files. Preserve the loaded skill's routing and never blanket-read substrates it does not require. For added or modified files, read target_sha:<path>; for deleted files, read base_sha:<path>; for renames, read both base_sha:<old-path> and target_sha:<new-path>. Never use the moving working-tree copy as review evidence or substrate.
   ...
   If the review is INCOMPLETE, put the INCOMPLETE line first, followed immediately by the same full target_sha and base_sha lines, `skill requested: <skill>`, and `skill source: <skill-source>` when a source was available. Return the loaded skill's Markdown report after this required prefix.
   ...
   The repo-owned `<skill>` and `<physical-skill>` values are both `lfx-self-serve-code-review` or both `lfx-self-serve-learnings-review`; their standard discovery aliases are `.claude/skills/local-code-review` and `.claude/skills/local-learnings-review`, but aliases are discovery pointers, not moving-worktree review inputs. For repo-owned children, `<skill-source>` is `git show <target_sha>:.claude/skills/<physical-skill>/SKILL.md` or `named Skill-tool load verified equivalent to that target blob`; for the general child it is `installed central plugin`. For normal review, `<range-label>` is `the latest commit`; for a coalesced batch it is `the commits since the last review`. Append `extra: <focus>` only for a real priority hint. Do not pass `branch` in post-commit mode.

5. **Validate the complete batch before accepting it.** Compare every report's assigned skill and full target/base SHA prefix with the pins. For each repo-owned child, also verify that its reported source is the target blob or a Skill-tool load shown equivalent to that blob. Any omission or mismatch invalidates the whole batch and leaves the last successfully reviewed target unchanged. Before the final-commit path begins, relaunch all three under the same pins if the branch has not moved, or coalesced from that unchanged marker through current `HEAD` if it has. After the final-commit path begins, do not relaunch an invalid earlier post-commit batch; the independently re-pinned cumulative full-branch sweep replaces it and covers the final commit plus all earlier branch work. When a batch is valid, advance the marker to its pinned `target_sha`.
...
2. **Run the full-branch sweep** when the branch has more than one commit, the final-commit optimization was used (mandatory then even on a single-commit branch), or current `HEAD` differs from the last successfully reviewed target. Drain all post-commit work first. Run `git fetch origin`, then pin `target_sha=$(git rev-parse HEAD)` and `base_sha=$(git merge-base origin/main HEAD)`. Every sweep launch or relaunch independently runs this fetch and re-pins that full cumulative range; never use the post-commit coalesced last-successfully-reviewed-target boundary as a sweep base. Launch the same three children with the canonical prompt, `<branch-line>` = `branch`, and `<range-label>` = `the branch's diff against origin/main`. Apply the same prefix/range validation and whole-batch invalidation rule. Address findings, then rerun one complete sweep until clean or explicitly documented as a trade-off.
...
4. **Run `/preflight`** for mechanical validation. Any commit or history rewrite created by the sweep, readiness, or preflight returns directly to step 2 — never launch a separate post-commit trio for it.
